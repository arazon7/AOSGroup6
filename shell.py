...
Available commands:
  ...
  ── Pipelines & Filters ──────────
  grep [-i] [-v] <pattern> [file]  Search for pattern in input or file
  sort [-r]          Sort lines of input (ascending; -r reverses)
  wc [-l|-w|-c]      Count lines (-l), words (-w), or chars (-c)
...
def run_builtin(args, input_data=None):
    ...
    # ── cat ───────────────────────────────────────────────────────────────────
        ...
        if len(args) < 2:
            # No file argument — pass piped input straight through
    ...
    # ── grep ──────────────────────────────────────────────────────────────────
        ...
        # Cross-platform builtin so piping works on Windows too.
        import re as _re
        flag_args = [a for a in args[1:] if a.startswith("-")]
        ...
        pattern     = non_flag[0]
        files       = non_flag[1:]
        ignore_case = "-i" in flag_args
        invert      = "-v" in flag_args
        re_flags    = _re.IGNORECASE if ignore_case else 0

        try:
            regex = _re.compile(pattern, re_flags)
        ...
        if files:
            for fname in files:
                if not has_permission(fname, "r"):
                    return "permission denied\n"
                try:
                    with open(fname, encoding="utf-8") as f:
                        lines_to_search.extend(f.readlines())
                except FileNotFoundError:
                    return f"grep: {fname}: file not found\n"
                except Exception as e:
                    return f"grep: {fname}: {e}\n"
        elif input_data:
            lines_to_search = input_data.splitlines(keepends=True)
            # Ensure last line has newline for consistent output
            if lines_to_search and not lines_to_search[-1].endswith("\n"):
                lines_to_search[-1] += "\n"
        else:
            return ""
    ...
    # ── sort ──────────────────────────────────────────────────────────────────
        ...
        # Usage: sort [-r] — sorts lines of piped input or a file
        non_flag = [a for a in args[1:] if not a.startswith("-")]
        reverse  = "-r" in args
        ...
        if non_flag:
            ...
            try:
                ...
                return f"sort: {fname}: file not found\n"
            except Exception as e:
                return f"sort: {e}\n"
        elif input_data:
            source = input_data
        ...
        # Default: case-insensitive (so Readme.md, abc.txt, .git sort naturally).
        # Pass -f to force case-sensitive (strict ASCII/byte order).
        case_sensitive = "-f" in args
    ...
    # ── wc ────────────────────────────────────────────────────────────────────
        ...
        if non_flag:
            ...
            try:
                ...
                return f"wc: {fname}: file not found\n"
            except Exception as e:
                return f"wc: {e}\n"
        elif input_data:
            source = input_data
    ...
    # ── head ──────────────────────────────────────────────────────────────────
        ...
        if non_flag:
            ...
            try:
                ...
                return f"head: {fname}: file not found\n"
            except Exception as e:
                return f"head: {e}\n"
        elif input_data:
            source = input_data
    ...
    # ── tail ──────────────────────────────────────────────────────────────────
        ...
        if non_flag:
            ...
            try:
                ...
                return f"tail: {fname}: file not found\n"
            except Exception as e:
                return f"tail: {e}\n"
        elif input_data:
            source = input_data
...
def run_pipeline(command_line):
    """
    Execute a pipe-separated command line.

    Each stage's stdout becomes the next stage's stdin.
    Built-in commands that produce output return it as a string so it flows
    through the pipe chain.  External commands are run with subprocess using
    the accumulated string as stdin.
    """
    stages = split_pipeline(command_line)
    if not stages:
        return None

    data = None   # accumulated stdout between stages
    ...
    for stage_idx, stage in enumerate(stages):
        ...
        try:
            ...
            print(f"pipeline: parse error in '{stage}': {e}")
            return None
        ...
        if cmd in BUILTINS:
            output = run_builtin(args, input_data=data)
            # Builtins return a string, or None if they acted directly (e.g. clear).
            # Treat None as empty string so the pipeline doesn't break.
            data = output if output is not None else ""
        else:
            # External command — feed accumulated data as stdin
            try:
                result = subprocess.run(
                    stage,
                    input=data,
                ...
                data = result.stdout
                # Surface stderr so users see error messages from external tools
                if result.stderr:
                    print(result.stderr, end="")
            except Exception as e:
                print(f"pipeline: error running '{stage}': {e}")
                return None
